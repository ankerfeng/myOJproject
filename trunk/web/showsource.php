<?php
	...
	if (!isset($_GET['id'])){
		...
		$sproblem_id=strval(intval($sproblem_id));
 		$sql="SELECT `title` FROM `problem` where `problem_id`='".$sproblem_id."'";
		$resultsf=mysql_query($sql);
		$rowsf=mysql_fetch_object($resultsf);
		$sproblemname=$rowsf->title;
		mysql_free_result($resultsf);
