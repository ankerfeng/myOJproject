       ...
       if ($row['contest_id']>0) {
        ...
        if (intval($row['result'])==11 && ((isset($_SESSION['user_id'])&&$row['user_id']==$_SESSION['user_id']) || isset($_SESSION['source_browser']))){
              ...
              if(!$lock||$lock_time>$row['in_date']||$row['user_id']==$_SESSION['user_id']){
                        ...
                        $view_status[$i][3].= "<span>".$judge_result[$row['result']]."</span>";
                }
          }else{
              echo "<td>----";
          }
        ...
        if(isset($_SESSION['http_judge'])) {
       //           $view_status[$i][3].="<form method=post action=admin/problem_judge.php><input type=hidden name=sid value='".$row['solution_id']."'>";
                  $view_status[$i][3].="<select length=4 name=result>";
                  ...
                  $view_status[$i][3].="<input title='$MSG_Explain' type='text' name='explain'>";
                  $view_status[$i][3].="<input type=submit name=manual value='$MSG_OK'>";
                  $view_status[$i][3].="</form>";
        }                                    
        if ($flag){

                if ($row['result']>=4){
                        $view_status[$i][4]= "<div>".$row['memory']."<div>";
                        $view_status[$i][5]= "<div>".$row['time']."</div>";
			//echo "=========".$row['memory']."========";
                }else{
                        $view_status[$i][4]= "---";
                        $view_status[$i][5]= "---";
			...
			//echo $row['result'];
                if (!(isset($_SESSION['user_id'])&&strtolower($row['user_id'])==strtolower($_SESSION['user_id']) 
			|| isset($_SESSION['source_browser']))){
